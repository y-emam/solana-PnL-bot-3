...
const groupTransactions = (transaction_ledger, wallet) => {
  ...
  ROI =
    parseFloat((Number(totalReturn) / Number(totalInvested)).toFixed(2)) + "x";
  ROIP =
    parseFloat((Number(totalReturn) / Number(totalInvested)).toFixed(2)) * 100 +
    " %";
  if (
    totalReturn > 0 &&
    winRate > process.env.WINRATE &&
    totalTokens > process.env.TOTALTOKEN
  ) {
    updateFinalCSV(
      wallet,
      positiveProfitCount + " / " + totalTokens + " = " + winRate.toFixed(2),
      `${totalInvested.toFixed(2)} USD`,
      `${totalReturn.toFixed(2)} USD`
    );
  }
