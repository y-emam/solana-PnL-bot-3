...
const groupTransactions = (transaction_ledger, wallet) => {
  ...
  transaction_ledger.forEach((tx) => {
    ...
    groupedTransactions[token_name][type].count += 1; // Increment count for the type

    if (!groupedTransactions[token_name][type].first_timestamp) {
      groupedTransactions[token_name][type].first_timestamp = timestamp;
    }
  });
  ...
  Object.keys(groupedTransactions).forEach((token_name) => {
    ...
    if (
      buyData.total_sol_balance === 0 &&
      buyData.total_usd_balance === 0 &&
      buyData.total_token_balance === 0
    ) {
      return;
    }
    ...
    // const tradeHoldTime = sellData.first_timestamp && buyData.first_timestamp
    //     ? ((new Date(sellData.first_timestamp) - new Date(buyData.first_timestamp)) / (1000 * 60 * 60)).toFixed(2) + " hours"
    //     : "N/A";

    const tradeHoldTime = formatTime(
      Math.abs(
        new Date(buyData.first_timestamp) - new Date(sellData.first_timestamp)
      ) / 1000
    );
    ...
    csvRows.push([
      ...
      buyData.count, // Include number of buy transactions
      tradeHoldTime,
      "",
      "",
      "",
    ...
    csvRows.push([
      ...
      sellData.count, // Include number of sell transactions
      tradeHoldTime,
      "",
      "",
      "",
  ...
  const winRate =
    totalTokens > 0 ? (positiveProfitCount / totalTokens) * 100 : 0;
  ...
  const ROI = parseFloat((totalReturn / totalInvested).toFixed(2)) + "x";
  const ROIP =
    parseFloat((totalReturn / totalInvested).toFixed(2)) * 100 + " %";
    ...
    while (true) {
      ...
      if (transactions && transactions.length > 0) {
        for (var i = 0; i < transactions.length; i++) {
          ...
          if (transactions[i].description.length == 0) continue;
          const tokens = parseTransaction(transactions[i]);

          if (tokens[1] != "SOL" && tokens[4] != "SOL") continue;
          if (tokens[1] == "SOL" && tokens[4] == "SOL") continue;
...
async function main(wallet) {
  ...
  Object.keys(ledger).forEach((key) => {
    ...
    if (!isNaN(token_sum)) sum += token_sum;
  });
  //const win_rate_percent = 100.0 * Object.keys(final_ledger).length / Object.keys(ledger).length;
  const win_rate_percent =
    (100.0 * win_count) / Object.keys(final_ledger).length;
  const winRate = `${
...
//     let rows = [];
//     rows.push(["Wallet Address", "Win Rate", "PNL", "Total Transactions"]);
...
//         // Write to final csv
//         rows.push([res.wallet, res.winRate, res.totalPnL, res.totalCount]);
//         fs.writeFileSync(
//           "output.csv",
//           rows.map((row) => row.join(",")).join("\n")
//         );
